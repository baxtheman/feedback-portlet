...
window.Feedback.Form = function( options ) {
	...
	options.descriptionLabel = options.descriptionLabel || "Please describe the issue you are experiencing";
	...
	for (; i < len; i++) {
		...
		switch( item.type ) {
			case "textarea":
				...
				break;
			case "button":
				button = element( "button", item.label );
				button.onclick = item.on;
...
window.Feedback.Form.prototype.review = function( dom ) {
	...
	for (; i < len; i++) {
		...
		if (item.element !== undefined && item.element.value.length > 0) {
...
window.Feedback.Screenshot.prototype.start = function( modal, modalHeader, modalFooter, nextButton ) {
	...
	if ( this.h2cDone ) {
		...
		var img = new Image();
		img.src = "/feedback-portlet/images/ts.gif";
		img.style.float = "left";
		img.style.margin = "5px";

		this.dom.appendChild( img );

		this.dom.appendChild( element("p", this.options.selectAreaLabel) );
